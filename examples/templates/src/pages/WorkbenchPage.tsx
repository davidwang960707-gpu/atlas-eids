    ...
    <div className="dashboard-grid">
      <Panel title="工作负载趋势" description="任务吞吐与 AI 协同处理量" action={<AtlasTag intent="success">实时</AtlasTag>} className="chart-panel">
        <div className="chart-wrap" role="img" aria-label="过去七日工作负载面积图">
          <ResponsiveContainer width="100%" height="100%"><AreaChart data={trend} margin={{ top: 10, right: 6, left: -22, bottom: 4 }}><defs><linearGradient id="tasks-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#7B61FF" stopOpacity=".28"/><stop offset="1" stopColor="#7B61FF" stopOpacity=".02"/></linearGradient></defs><CartesianGrid strokeDasharray="3 5" vertical={false} stroke="var(--atlas-color-border-default)"/><XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fill: 'var(--atlas-color-text-tertiary)', fontSize: 12 }}/><YAxis axisLine={false} tickLine={false} tick={{ fill: 'var(--atlas-color-text-tertiary)', fontSize: 12 }}/><Tooltip contentStyle={{ borderRadius: 6, borderColor: 'var(--atlas-color-border-default)' }}/><Area isAnimationActive={false} type="monotone" dataKey="tasks" stroke="#7B61FF" strokeWidth={2} fill="url(#tasks-fill)" activeDot={{ r: 4 }}/><Area isAnimationActive={false} type="monotone" dataKey="ai" stroke="#06B6D4" strokeWidth={2} fill="transparent" activeDot={{ r: 4 }}/></AreaChart></ResponsiveContainer>
        </div>
