...
Atlas EIDS 采用 Monorepo 管理 Design Tokens、框架无关内核、插件、第三方适配器、AI Runtime、网页 Agent、源码生成器以及 React / Vue 组件包。
...
Core Contracts ── Plugin SDK ── Third-party Adapters
    ↓                    ↓
React / Vue UI      AI Runtime ── Web Agent
    ↓                    ↓
Page Patterns       Tools / Approval / Audit
    └──────── Source Generator ────────┐
                                      ↓
                           React / Vue + Java App
```
...
2. React 与 Vue 组件保持相同的状态语义、可访问名称和视觉类名，框架 API 可以符合各自习惯。
3. 高风险 Tool Call 默认返回 `approval-required`，不得静默执行。
4. 第三方适配器不得复制第三方组件源码，只维护语义 Token、组件映射和组合层。
5. 源码生成器输出可独立维护的项目，不要求应用运行时依赖生成器。
6. Living Orb 仅表示 AI 身份或运行状态，不能装饰普通业务对象。

## 当前边界

- Pattern Lab 的 126 个条目是页面模式与交互参考，并非 126 个独立可发布组件。
- 第三方运行时 Wrapper 已覆盖核心表单、数据与浮层控件；复杂事件归一化和高级组件兼容仍在建设。
- Web Agent 已有权限化工具注册和 Model Context Bridge，但尚未宣称完整实现 WebMCP 标准。
